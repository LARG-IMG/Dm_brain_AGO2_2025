...
for bam_file in $BAM_DIR/E*DamHP1*.bam; do
    # Extract the base name of the file without extension
    base_name=$(basename "$bam_file" .map.bam)
    ...
    dam_file="${BAM_DIR}/${prefix}_Dam_dm3.map.bam"
    ...
    if [ -f "$dam_file" ]; then
        ...
        output_file_uniq="${OUT_DIR}/${base_name}__Dam_norm_uniq.bedgraph"
	output_file_multi="${OUT_DIR}/${base_name}_Dam_norm_multi.bedgraph"
